...
public class StadiumSeatButton extends JButton implements Serializable {
					...
					try {
						...
						if (!strutturaSportiva.verificaPrenotazione(cliente, partita)) {
							...
							JOptionPane.showMessageDialog(StadiumSeatButton.this,
									...
									"Prenotazione gi� presente", JOptionPane.ERROR_MESSAGE);
						}
					} catch (AlreadyExistsObjectException e2) {
						JOptionPane.showMessageDialog(null, e2.getMessage(), "Acquisto gi� presente Exception.",
								JOptionPane.ERROR_MESSAGE);
					} catch (PostoIndisponibileException e1) {
	...
	/**
	 * Controlla se il posto � indisponibile o meno.
	 * 
	 * @return true se il posto � indisponibile, false altrimenti.
	 * @author Maurizio
	 */
	...
	/**
	 * Restituisce la modalit� di accesso con cui si chiede di visualizzare il
	 * posto.
	 * 
	 * @return La modalit� di accesso con cui si chiede di visualizzare il
	 *         posto.
	 * @author Maurizio
	 */
